/* ============================================
   PRACTICE-PAGE.JS — Dedicated Practice handler
   ============================================ */
...
(function() {
  ...
  async function loadSets() {
    try {
      var sets = await ManifestLoader.getPracticeSets();
      state.sets = sets || [];
  ...
  function createCard(set, number, progress) {
    ...
    card.className = 'set-card set-card--practice' +
    ...
    card.innerHTML =
      ...
      '<div class="set-card__sub">' + (set.topic || 'Practice Set') + '</div>' +
      '</div>' +
      '</div>' +
  ...
  function startQuiz(set) {
    ...
    var params = 'setId=' + set.id + '&type=practice&mode=' + mode;
